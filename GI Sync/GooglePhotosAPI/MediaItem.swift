...
class MediaItem: Mappable {
    ...
    func download(completed: @escaping (_ image: NSImage) -> Void, failed: @escaping (_ error: Error) -> Void) {
        if let url = self.baseUrl,
            ...
            Alamofire.request(
                fullSizeUrl,
                headers: AuthenticationHelper.authorizationHeader()).responseData { (response: DataResponse<Data>) in
    ...
    static func list(
        ...
        Alamofire.request(
            ...
            headers: AuthenticationHelper.authorizationHeader()).responseObject { (response: DataResponse<MediaItemsResponse>) in
    ...
    static func get(
        ...
        Alamofire.request(
            url,
            headers: AuthenticationHelper.authorizationHeader()).responseObject { (response: DataResponse<MediaItem>) in
            if response.response?.statusCode == 200, let result = response.result.value {
                completed(result)
            } else {
                ErrorResponse.handleErrorResponse(response: response, failed: failed)
